			...
			.then(data => {
				...
				data.forEach(flight => {
					...
					row.innerHTML = `
						...
						<td>${Number.parseFloat(flight.last_latitude).toFixed(5)}</td>
						<td>${Number.parseFloat(flight.last_longitude).toFixed(5)}</td>
						<td>${Math.trunc(flight.last_altitude)}</td>
						<td>
...
var map;
function mapInit() {
	map = L.map('map-container').setView([52.114339, 19.423672], 6);
